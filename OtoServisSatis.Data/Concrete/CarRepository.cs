...
namespace OtoServisSatis.Data.Concrete
{
    public class CarRepository : Repository<Arac>, ICarRepository
    {
        ...
        {
            return await _dbSet.AsNoTracking().Include(x => x.Marka).FirstAsync(c=>c.Id==id);//Bir tane kayıt döndürecek
        }

        public async Task<List<Arac>> GetCustomCarList()
        {
            return await _dbSet.AsNoTracking().Include(x=>x.Marka).ToListAsync();//Marka'yı dahil et
        }
